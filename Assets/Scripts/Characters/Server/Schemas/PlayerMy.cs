...
public PlayerMy() { }
	...
	[Type(5, "ref", typeof(Vector3D))]
	public Vector3D weaponRot = new Vector3D();
	...
	protected event PropertyChangeHandler<Vector3D> __weaponRotChange;
	public Action OnWeaponRotChange(PropertyChangeHandler<Vector3D> __handler, bool __immediate = true) {
		if (__callbacks == null) { __callbacks = new SchemaCallbacks(); }
		__callbacks.AddPropertyCallback(nameof(this.weaponRot));
		__weaponRotChange += __handler;
		if (__immediate && this.weaponRot != null) { __handler(this.weaponRot, null); }
		return () => {
			__callbacks.RemovePropertyCallback(nameof(weaponRot));
			__weaponRotChange -= __handler;
		};
	}
	...
	protected override void TriggerFieldChange(DataChange change) {
		switch (change.Field) {
			...
			case nameof(weaponRot): __weaponRotChange?.Invoke((Vector3D) change.Value, (Vector3D) change.PreviousValue); break;
			default: break;
		}
	}
}
